...
def cancel_tasks(tasks):
    ...
    if tasks=='all':
        try:
            ...
            all_tasks = [task for task in ee.data.getTaskList() if task['state'] == 'RUNNING' or task['state']=='READY']
            if len(all_tasks)>0:
                for task in all_tasks:
                    ee.data.cancelTask(task['id'])
                print('Request completed task ID or task type {} cancelled'.format(tasks))
        ...
        try:
            ...
            ready_tasks = [task for task in ee.data.getTaskList() if task['state'] == 'READY']
...
def quota():
    ...
    for roots in ee.data.getAssetRoots():
        ...
        print('')
        print('Root assets path: {}'.format(roots['id'].replace('projects/earthengine-legacy/assets/','')))
        print('Used {} of {}'.format(humansize(quota['asset_size']['usage']),humansize(quota['asset_size']['limit'])))
        print('Used {:,} assets of {:,} total'.format(quota['asset_count']['usage'],quota['asset_count']['limit']))
...
def assetsize(asset):
    ...
    if header=="IMAGE_COLLECTION":
        ...
        b=subprocess.check_output("earthengine --no-use_cloud_api du "+asset+" -s",shell=True).decode('ascii')
        num=subprocess.check_output("earthengine --no-use_cloud_api ls "+asset,shell=True).decode('ascii')
...
def main(args=None):
    ...
    parser_search = subparsers.add_parser('search',help='Search public GEE catalog using keywords')
    required_named = parser_search.add_argument_group('Required named arguments.')
    required_named.add_argument('--keywords', help='Keywords to search for can be id, provider, tag and so on', required=True)
    ...
    required_named.add_argument('--outfile', help='This it the location of your report csv file ', required=True)
    parser_ee_report.set_defaults(func=ee_report_from_parser)

    parser_assetsize = subparsers.add_parser('assetsize',help='Prints any asset size (folders,collections,images or tables) in Human Readable form & Number of assets included')
    required_named = parser_assetsize.add_argument_group('Required named arguments.')
    required_named.add_argument('--asset', help='Earth Engine Asset for which to get size properties', required=True)
    ...
    required_named.add_argument('--tasks', help='You can provide tasks as running or ready or all or even a single task id', required=True,default=None)
    parser_cancel.set_defaults(func=cancel_tasks_from_parser)

    parser_copy=subparsers.add_parser('copy',help='Copies entire folders, collections, images or tables')
    ...
    parser_move=subparsers.add_parser('move',help='Moves entire folders, collections, images or tables')
    required_named = parser_move.add_argument_group('Required named arguments.')
    required_named.add_argument('--initial',help='Existing path of assets')
    required_named.add_argument('--final',help='New path for assets')
    parser_move.set_defaults(func=move_from_parser)

    parser_access = subparsers.add_parser('access',help='Sets Permissions for entire folders, collections, images or tables')
    required_named = parser_access.add_argument_group('Required named arguments.')
    required_named.add_argument('--asset', help='This is the path to the earth engine asset whose permission you are changing folder/collection/image', required=True)
    required_named.add_argument('--user', help='"user:person@example.com" or "group:team@example.com" or "serviceAccount:account@gserviceaccount.com", try using "allUsers" to make it public', required=True, default=False)
    required_named.add_argument('--role', help='Choose between reader, writer or delete', required=True)
    parser_access.set_defaults(func=access_from_parser)

    parser_delete = subparsers.add_parser('delete', help='Deletes folders or collections recursively')
    required_named = parser_delete.add_argument_group('Required named arguments.')
    required_named.add_argument('--id', help='Full path to asset for deletion. Recursively removes all folders, collections and images.',required=True)
    ...
    required_named = parser_delete_metadata.add_argument_group('Required named arguments.')
    required_named.add_argument('--asset', help='This is the path to the earth engine asset whose permission you are changing collection/image', required=True)
    required_named.add_argument('--property', help='Metadata name that you want to delete', required=True, default=False)
