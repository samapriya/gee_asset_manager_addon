...
def lst(location, typ, items=None,output=None):
    if items > 0 and typ =='print':
        b=subprocess.check_output("earthengine ls "+'"'+location+'"'+" --max_items "+str(items)+" -l -r",shell=False)
        try:
            for item in b.split('\n'):
        ...
        b=subprocess.check_output("earthengine ls "+'"'+location+'"'+" -l -r",shell=False)
        try:
            for item in b.split('\n'):
                a=item.replace("[","").replace("]","").split()
                header=a[0]
